...
| `pnpm check:templates` | `pnpm check:all` |
...
**`check:templates` in one paragraph** (task 037). `create-qcms-app` ships the owned application shell an adopter receives, and that shell has to BE the one this repository develops or the CLI quietly becomes a fork nobody runs. So `packages/create-qcms-app/templates/` is never hand-edited: `pnpm qcms:sync-templates` derives it from `apps/`, `docker/`, `docker-compose.yml` and the configuration schema by a declared set of transforms (tests dropped, `workspace:*` resolved to a real version range, monorepo-relative paths resolved through `node_modules`, `.env.example` generated from `ENV_REFERENCE`), and this gate regenerates into memory and byte-compares. Change an app shell file without regenerating and it fails naming the file. Generation is deliberately NOT wired into `build`: turbo hashes each package's own inputs, `apps/**` is not among `create-qcms-app`'s, so a cached build would skip regeneration after exactly the change that needed it.

**Why the scaffolding e2e is separate** (task 037). `create-app-e2e` packs the four `@qcms/*` packages and the CLI with `pnpm pack`, runs the CLI from its own tarball into a temporary directory, installs with **no workspace links**, builds the three images from the scaffolded tree, starts the stack, and drives the 027 scenario-1 loop over HTTP against it. That is the only gate in which none of pnpm's workspace hoisting is available, which is precisely what makes it able to see an undeclared dependency, a missing `files` entry, or a path that only resolves in a monorepo. It builds three images, so it stays out of `verify`; run it locally with `QCMS_PORT_SEAT=<0-9> pnpm qcms:scaffold-e2e` (self-contained: pack, scaffold, up, suite, teardown) when you change `packages/create-qcms-app`, an app's `package.json`, a Dockerfile, or anything either app reads from the environment at boot. It takes this seat's **harness** ports and its own Compose project, so it never shares a seat with `verify:browser` or `up:e2e`.

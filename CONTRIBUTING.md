...
| `compose-e2e` job in `e2e.yml` (`pnpm docker:up` + `pnpm test:e2e`) | **`pnpm up:e2e`** - deliberately not in `verify` or `verify:browser`, see below |

...
**It runs from the dev container as of issue #316, and did not before.** The stack publishes on the Docker host, which inside the dev container is not this process (`docker compose` drives the mounted host socket, ADR-29). The harness used to dial a hardcoded `localhost` and died in `beforeAll` with `ECONNREFUSED` while every container reported healthy, so `pnpm up:e2e` was effectively CI-only from the canonical environment. It now resolves the default-route gateway, publishes on that same interface, and turns off the admin's `Secure` cookie flag, because a `Secure` cookie can only be stored by a trustworthy origin: `http://localhost` is one, a bare gateway IPv4 is not, so the browser would drop the session cookie and sign-in would appear to succeed and bounce. Those three move together or not at all (`scripts/compose-e2e.mjs`'s `composeEnvironmentOverrides`, `scripts/docker-host.mjs`, `docs/PORTS.md`). A plain host checkout and CI are unchanged at `localhost`, loopback, and cookies left to `NODE_ENV`. If you ever need to override the address by hand, `QCMS_DOCKER_PUBLISH_HOST` wins over the detection.

...
**`check:security-hygiene` in one paragraph** (task 040). Three static properties the runtime controls cannot establish about themselves. **Answer content is never passed to a logger**: the stdout logger redacts by field name and the OTLP path is an allowlist, so a value logged under a key nobody anticipated still reaches an operator's aggregator, and SEC-8 states the rule at the call site ("log questionIds and counts, not content"). **No committed `.env*.example` carries a live-looking secret**, which is the mirror of the boot-time refusal in `apps/api/src/config.ts`: the config guard stops a placeholder reaching production, this gate stops a real secret reaching the repository, and **the two placeholder lists must stay aligned or a spelling one accepts and the other rejects reopens the gap**. **No SQL is built by string concatenation**: Drizzle's `sql` tagged template binds its interpolations, `sql.raw()` does not, and a template literal handed to `execute`/`query` is the same hole wearing different clothes. One line is waived with a `check-security-hygiene: allow <reason>` comment directly above it, so the reason sits in the diff where a reviewer sees it. **Adding a gate to `check:all` does not put it in CI**: `.github/workflows/ci.yml` enumerates every check as its own step, and three changes in one day (PR #451, PR #413, and this task) shipped a gate that ran locally and never in CI (issue #463). The gate's self-tests run under the `tooling` vitest project; the repository scan is the workflow step.

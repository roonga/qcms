...
| --- | --- | --- | --- |
| `postcss: ^8.5.23` | GHSA-qx2v-qp2m-jg93, GHSA-6g55-p6wh-862q, GHSA-r28c-9q8g-f849 | Next pins `postcss` at exactly `8.4.31`; `8.5.23` is the first release past all three patched ranges and stays inside postcss 8 | Next's own pin reaches `>= 8.5.18` |
| `minimatch@5: ^10.2.5` · `minimatch@9: ^10.2.5` | GHSA-mh99-v99m-4gvg (indirectly) | minimatch 5/9 pin `brace-expansion@^2`, and the fix exists only in `brace-expansion` 5.0.8; minimatch 10 is the first line on `^5`. Safe because every dependent (`glob@10`, `readdir-glob@1`) imports minimatch by **named** export, which 10 keeps | `archiver`/`testcontainers` reach minimatch 10 on their own |
| `brace-expansion@5: ^5.0.8` | GHSA-mh99-v99m-4gvg | 5.0.8 is the only fixed release; the 1.x/2.x/3.x maintenance lines have no backport. Scoped to `5` so a future `^2` consumer is not forced onto 5.x, whose CJS build exports `{ expand }` and is **not** callable | minimatch's own floor reaches 5.0.8 |

...
**It runs from the dev container as of issue #316, and did not before.** The stack publishes on the Docker host, which inside the dev container is not this process (`docker compose` drives the mounted host socket, ADR-29). The harness used to dial a hardcoded `localhost` and died in `beforeAll` with `ECONNREFUSED` while every container reported healthy, so `pnpm up:e2e` was effectively CI-only from the canonical environment. It now resolves the default-route gateway and publishes on that same interface (`scripts/docker-host.mjs`, `docs/PORTS.md`). A plain host checkout and CI are unchanged at `localhost`. If you ever need to override the address by hand, `QCMS_DOCKER_PUBLISH_HOST` wins over the detection.

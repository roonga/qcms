 ...
 * U+2013) is allowed for numeric ranges (e.g. "R1–R7", "028–035"); the hyphen
 * (-) is always fine.
 *
 * Deny-by-default over tracked .md / .ts (vendored a2ra components excluded).
...
function tracked() {
  const out = execFileSync(
    GIT,
    ["ls-files", "-z", "*.md", "*.ts", ":!packages/ui/src/components/**"],
    { encoding: "utf8" },
  );
...
if (violations.length > 0) {
  console.error("check-no-em-dash: em dash (—) is banned in QCMS prose/comments/strings:\n");

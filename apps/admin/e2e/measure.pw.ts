 ...
 * is what this spec is for: it opens all eighteen screens and measures the shell's content
 * column.
 *
 ...
 *   That is issue 648's acceptance, and it is asserted on all eighteen rather than sampled
...
/**
 * All eighteen authenticated screens, in route order.
 *
...
function screens(sessionId: string): readonly Screen[] {
  return [
    ...
    // where a reader looking for it will look; `rules-screen-poc.html` caps `.main` at
...
test("657 caps each screen at the width its own POC draws, at 1280", async ({ page }) => {
  ...
  for (const screen of screens(sessionId)) {
    ...
    // Soft, so one sweep reports all eighteen verdicts instead of stopping at the first
...
test("657 leaves every screen the same width at 390, because a cap is fluid below itself", async ({
  ...
  expect(new Set(widths.values()).size, "all eighteen screens measure one width at 390").toBe(1);
});

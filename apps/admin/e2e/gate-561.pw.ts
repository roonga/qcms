...
async function capture(page: Page, name: string, frame: Frame): Promise<void> {
  ...
  //
  // The builder frame used to carry a one-pixel allowance here, spelled out with its
  // reason: the builder scrolled sideways by a pixel at 390 and that was issue 616,
  // open and out of this issue's scope. Issue 616 has since fixed it, so the allowance
  // is gone and every frame in this set is held to its own viewport. A declared
  // exemption that stops being needed is a defect of the test that declares it.

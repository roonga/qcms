...
function adminAuthConfig(
  ...
  return {
    ...
    // (better-auth 1.7.5, the pinned version, `dist/api/index.mjs:165-173`).
    // <!-- expect: const rateLimitResponse = await onRequestRateLimit(currentRequest, ctx); -->
    // Stating the default rather than the escape hatch keeps this fixture from quietly
    // becoming a place the control is off for no reason.
    signInThrottle: true,
  };
}

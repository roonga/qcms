...
${block(":root", { ...light, ...spacing, "--font-admin": `ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, sans-serif`, "--font-mono": `ui-monospace, "SF Mono", "Cascadia Code", "Roboto Mono", Consolas, monospace` })}
...
portal theme system applied at the admin level: the admin wears sharp, the
portal keeps Subtle.
...
- **One system, two seats.** The admin reuses the portal's slate neutrals and
  ...
  chrome language from the design system overview. The one deliberate
  divergence is Corners: the DS overview and the portal's Subtle theme use an
  8px radius base, but the admin (rev 4) sharpens to a 4px/8px/2px
  (control/card/small) industrial scale - an intentional admin-level brand-
  character choice, not drift. Everything else in this theme is the DS applied
  to the admin surface, unchanged.

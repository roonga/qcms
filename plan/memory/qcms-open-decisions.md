...
Also open (from Stage 6 audit): issue #5 (@roonga/qcms-db enum-row types) - spec'd, scheduled 030→031 via executor+reviewer. Harness Write-guard blocks task-declared doc/changeset deliverables - flagged to the Code Owner, harness-config level.
...
- **Managed theming (#26 / finding K):** predefined themes + customize + save named custom theme (`@roonga/qcms-db` storage + admin UI + apply to portal). Needs a **new ADR + task set**; open decisions: launch vs Phase-4, per-deployment/form/org granularity, mutable operator config vs versioned/immutable like published forms.

...
- **Local `turbo` binary is broken on the Windows host** (crashes 0xC0000135 even on `--version`) in the Windows-host `a2-react-aria` checkout as of 2026-07-20; host-Windows only - does not apply under WSL2/devcontainer (Linux binaries). Workaround there: run gates per package (`pnpm --filter @a2ra/core test|build`, same for cli). Proper fix suggested to the Code Owner: `pnpm install --force`. CI (Linux) unaffected.

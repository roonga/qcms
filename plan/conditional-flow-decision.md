...
- **Where logic runs:** the server. `@qcms/core`'s forward-pass evaluator is the only rule engine; the portal never evaluates (R2). "Question-level execution" can only mean _when the client posts an answer and when the UI applies the server's re-evaluation_.

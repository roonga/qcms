...
> **Ruled on, and the two halves went opposite ways.** Validation stayed on the builder page (issue #659, built as #719): this paragraph is the reason, and it stands. Rules moved to `/forms/{formId}/rules` (issue #669, Code Owner 2026-09-05, built 2026-09-06), with the two-hop path accepted knowingly in the sense this paragraph asks for - and with a condition attached, which is what makes the difference between the two decisions coherent. Every rule link carries the route as well as the fragment and the destination focuses the row on arrival, so the anchors were rebuilt rather than left to resolve to nothing. `plan/admin-design-contracts.md` §7 carries the ruling and what the build did with it.

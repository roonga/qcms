...
**This document is maintained, not a frozen snapshot** (§8 item 2). Findings are kept with a status note recording what closed them and what they left behind, rather than deleted: the entry plus its outcome is the useful record, and several entries turned out to understate what they had found. **Last swept 2026-08-22** against `origin/main` at `434d5c9`, re-reading each cited file rather than trusting the entry. The first sweep was written against `90f8fba` and went stale within hours: eleven PRs merged behind it, four of its own status notes became wrong, and it had to be swept again before it landed. A maintained document is not maintained by one pass.
...
## What this document is now, after the authority reversed (2026-08-22)

This audit was written on the premise that the contracts governed and the POCs were proposals. **That premise is gone**: the Code Owner ruled the POCs are the approved design, and `docs/admin-constraints.md` is the operative text.

**This document is evidence, not authority.** Its value is that it is the only place the POC's proposals were ever **costed** against shipped behaviour, file and line. Where it says a thing should not be done, read that as a price rather than a veto, and the price is usually the useful half:

- **"Do not split Validation onto its own route" (§5.5) still stands on its own evidence**, and is the basis of the recommendation on #659. That section also prices the other three extractions in the same rail - Rules can move at the cost of a two-hop path for rule-scoped issues (#669), Settings and Test bench move freely. Four routes, priced separately, before the authority reversed.
- **"Do not put a rail on ... `/settings`" is STALE.** Issue 562 shipped a Settings rail and §7a of `plan/admin-design-contracts.md` records it as a written exception. The reasoning in that bullet (six of sixteen screens have nothing to put in a rail) still holds for the screens it still covers.
- **"Do not make any one-time reveal collapsible" (§5.2) holds and is now confirmed shipped.** `components/forms/secure-links.tsx:164` is a `<section>`, and no `<details>` in the admin is a reveal.

## Remediation step three is complete: all eleven POCs compared

Every POC in `plan/admin-shell-poc/` has now been read against what shipped. What it found:

| POC | Outcome |
| --- | --- |
| `settings-newquestion-poc` | Rebuilt to it (#655 / PR #662) |
| `question-editor-poc` | Rail shipped to it (#650 / PR #670), including the §7 action block the Code Owner overruled by name |
...
**Done by issue 519**, with one thing the entry did not anticipate. §3.7 turned out to bind here even though this disclosure is a button rather than a `<details>`: latency's cell carries `qcms-cell--drop`, which is `display: none` below 40rem, so a digest naming latency would have been its only copy at a phone width. The panel therefore gained a `This delivery` list holding all three digested facts in full, which incidentally supplied the `h3` that closed the panel's registered heading-order gap (issue #541).
...
Native `<details>` in the builder (`components/forms/form-settings-panel.tsx:60`, `components/forms/rule-test-bench.tsx:69`) against a hand-built `aria-expanded` button in the delivery dashboard (`components/ops/delivery-dashboard.tsx:160-171`) and in the option grid's grip (`components/questions/option-grid-editor.tsx:459`). Both are defensible in place. What was not defensible is that neither builder `<details>` put a heading in its `<summary>`, so the Settings and Test bench panels had **no entry in the heading outline at all**, while every other section on the same page has an `h2`.

**Closed by issue 519.** Both summaries now hold an `h2` at the level every other section of the builder uses, and both carry a §3.7 digest beside it. The two disclosure idioms remain, and remain defensible in place: the finding was about the missing heading, not about the mechanism.
...
7. ~~**Element 3 on the builder's existing two `<details>` panels**, adding the digest and, separately, a heading in each `<summary>` so they stop being invisible in the outline (§4.3). Then the delivery dashboard's row trigger. Nowhere else yet.~~ **Done by issue 519**, in all three places and nowhere else. Two notes for whoever applies element 3 next. §3.7's "the fact must also exist inside the panel" is the clause that shapes the work, not the digest wording: it is what added a fact list to the delivery panel, and what kept a run outcome out of the test bench's digest (a verdict that only exists after a press would have lived in the summary alone). And §5.6 was satisfied by omission rather than by arithmetic - no digest states an issue count, so the validation panel's remains the screen's only one.

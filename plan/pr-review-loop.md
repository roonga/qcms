...
2. **Review the diff as a stranger.** `gh pr diff <NN>`. Checks, against the written rules (CI gates are a subset - `plan/**` is excluded from several):
   ...
   - **CI:** `statusCheckRollup` fully green. Local gate and CI are not supersets of each other (issue #19) - when a diff touches gate-adjacent files, run the root gate **and the CI-only gates listed in `.github/workflows/ci.yml`** (`check:no-em-dash`, `check:no-control-chars`, `check:licenses`, `check:duplication`, `check:golden-append-only`) in a temp worktree before merging.
   - **Existing review comments (Copilot included) - address every one before merging (Code Owner rule, 2026-07-25).** `gh api repos/<owner>/<repo>/pulls/<NN>/comments` and read the review bodies too. Each comment gets exactly one of: a fix pushed to the branch, or a reasoned reply on the thread explaining why not. Never merge a PR carrying an unaddressed review comment - silence reads as "nobody looked".

...
2. **Review the diff as a stranger.** `gh pr diff <NN>`. Checks, against the written rules (CI gates are a subset - `plan/**` is excluded from several):
   ...
   - **CI:** `statusCheckRollup` fully green - with one designed exception: the node-26 `verify` leg is `continue-on-error` by construction ("incoming LTS line - non-blocking early warning only", ci.yml). A red confined to that leg, with the identical suite green on node-24, may be waived; the waiver and the node-26 failure signature are recorded in the verdict comment, never silently. `pnpm verify` is the superset of CI's unit job (issue #19), so when a diff touches gate-adjacent files, run `pnpm verify` in a temp worktree before merging - and `pnpm verify:browser` as well for a diff touching `apps/portal`, `apps/admin`, or `@qcms/ui`, the one CI job `verify` omits.

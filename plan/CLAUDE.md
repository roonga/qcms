...
- **Does not:** implement product code. **Implementation goes through the dev loop** (`/task NNN`, `/next-task`, `scripts/agent-loop.sh` in the devcontainer once ADR-29/046 lands; `scripts/agent-loop.ps1` is the Windows-host fallback), launched from the **repo root** (the parent of this folder - `~/src/agent3/qcms` on WSL2, `H:\source\agent3\qcms` on the Windows host), not this seat.

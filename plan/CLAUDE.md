...
>
> **The admin screenshot gate is delegated to this seat for contract-governed work (Code Owner ruling, 2026-08-20).** A redesign PR that changes visible admin UI still **captures and commits** its frames under `docs/gates/pr-<NN>/`, but it no longer parks awaiting the Code Owner: this seat clears it against `plan/admin-design-contracts.md` (CONFIRMED 2026-08-20) and the frozen cards in `plan/admin-theme/`. **The POCs are not the reference** - they answer the same eight questions two to seven different ways, which is why the contracts exist; a POC is evidence of intent where a contract is silent, never an authority against one. Escalate anyway when the render contradicts a contract, when nothing governs it, when it introduces a new pattern rather than applying one, or when it touches the **portal** - this delegation is admin-tier only, and every other human gate (030 a11y, 040 security, 038 launch, wireframe sign-off, `.archive`) is untouched.
>

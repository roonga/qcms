...
## Active workstreams (snapshot 2026-08-22 - ALWAYS RE-VERIFY against ledger/git/issues before acting)

> **Standing aim (Code Owner, 2026-08-19): the admin redesign tier comes first.** The twelve issues carrying it are labelled **`admin-redesign`** (#510-#515, #517-#522) and are drained ahead of every other open issue, in the tier order set out in `plan/admin-redesign-implementation-plan.md` §3a: bugs (#510, #511, #513, #520, #521) -> cleanup (#512, #515, #522) -> consolidation (#514) -> house patterns (#517, #518, #519). Two ordering notes ride with it: #514 lands before #517-#519, because the house patterns inherit whatever table and empty state it settles; and #519 and #520 touch the same delivery-dashboard row trigger, so whichever lands second rebases over the first.
>
> **Intake rule (Code Owner, 2026-08-20): defects found while working the tier join the tier.** They are labelled `admin-redesign` and they are in scope - the loop does not need to ask. But **they queue behind what is already dispatched**, because label priority alone would let an issue filed today preempt work that has been sequenced for a week. The order the loop selects in:
>
> 1. **Findings cycles on open PRs** - a live `PO-REVIEW: CHANGES-REQUESTED @<current head>` always outranks fresh selection.
> 2. **The original twelve**, in the tier order above.
> 3. **Wave 3** (`wave-3` label, #557, #558, #559, #561, #562, #563), in its dispatched order, with #517-#519 landing before #559.
> 4. **Finds made during the run**, in filing order, bugs before cleanup before enhancement.
>
> **The guard, because a self-extending campaign has no natural end:** if the tier grows for two consecutive sweeps without net progress on tiers 2 and 3, that is reported to the Code Owner rather than absorbed. It went twelve to twenty-five in a day - that is a healthy audit finding real defects, and it is also how "drain the label" quietly becomes a target that recedes faster than the loop advances. The number moving is fine; nobody noticing is not.
>
> **The one exception is security on a public surface.** `/next-issue`'s own priority order is `security` -> `bug` -> unlabeled -> `enhancement`, and this aim overrides it for everything except a `security` issue whose exposure is unauthenticated respondent-facing. Today that is exactly one issue, **#504** (the portal's origin belt fails closed for browsers that send no Fetch Metadata). The rest of the security cluster (#482, #488, #489, #491, #492) is staff-surface or documentation and waits behind the `admin-redesign` tier, as do the `workshop` items, which never enter the issue loop anyway.
>
> **DESIGN AUTHORITY REVERSED, 2026-08-22 (Code Owner): *"poc wins in all cases. this is the approved design."*** The paragraph that stood here said the opposite - *"The POCs are not the reference [...] never an authority against one"* - and it is kept below only so nobody re-derives it from an older copy. **It is wrong now and it was the shape of the campaign's founding error: importing a constraint the Code Owner had not set.**
>
> **`plan/admin-shell-poc/*.html` is the approved design.** `plan/admin-design-contracts.md` is **description and rationale**, useful for a screen no POC covers and for explaining why shipped code looks as it does. It never overrules a drawing. `docs/admin-constraints.md` is the operative constraint text for the admin and `docs/portal-constraints.md` for the portal; **they are separate apps with separate constraints and neither's rules may be carried to the other.** All design and technical limits on the admin are removed - JavaScript is available and a design may depend on it. WCAG 2.2 AA is an **aim** for the admin, not a blocking gate (2026-08-21).
>
> **A POC's silence is not a ruling, and there are four cases.** *Could not have spoken* (URL handling, server behaviour - shipped behaviour stands) · *spoke* (POC wins) · *spoke but the expression is unbuildable in context* (heading order, landmark naming - honour the intent, change the expression, say so) · *spoke ambiguously because of how the file is packed* (multi-screen files: **inner class wins where present**). The test a lane can run: *could a single standalone HTML file, on its own, have exhibited the property you are invoking?* Full text and worked examples in `plan/admin-design-contracts.md`.
>
> **The screenshot-gate delegation itself stands (Code Owner, 2026-08-20)**, with its reference swapped: a redesign PR that changes visible admin UI still **captures and commits** its frames under `docs/gates/pr-<NN>/`, and this seat clears it **against the screen's own POC**. Escalate when the render contradicts its POC, when no POC covers the screen, when it introduces a new pattern rather than applying one, or when it touches the **portal** - this delegation is admin-tier only, and every other human gate (030 a11y, 040 security, 038 launch, wireframe sign-off, `.archive`) is untouched.
>
> **Superseded, retained so it is recognisable rather than rediscovered:** *"this seat clears it against `plan/admin-design-contracts.md` (CONFIRMED 2026-08-20) and the frozen cards in `plan/admin-theme/`. The POCs are not the reference - they answer the same eight questions two to seven different ways [...] a POC is evidence of intent where a contract is silent, never an authority against one."* Both halves are now false: the authority is reversed, and this seat separately **withdrew** the claim that the eleven POCs contradict each other - each draws a distinct screen. The frozen cards in `plan/admin-theme/` are also not above a POC, and one of them (`ds-table.html`) turned out to contradict a contract that cited it as support (#681).
>
> **Do not read the UI-visible list as authoritative** (corrected 2026-08-20): it omitted #510 and cost a round trip. Judge per fix - does anything rendered move?
>
> **The previous aim is discharged, not overridden:** the 033 -> 034 -> 035 chain it named landed as PRs #245, #274 and #284, so a respondent can complete a form end to end and an author can see what came back. Nothing about it constrains selection any more; do not read the old wording (kept in git history) as live.
>
> **The admin train is done.** 031-035 all landed, as did 047 (via 051/052/053), 048, 050, 054, 055 and 057. Verified against the ledger at `c9a5219`, not recalled. What is still `todo`: **037** (stage 8b CLI), **041** (agent authoring, flag-gated, never gates 038) and **049**. The claim protocol changed 2026-07-31: the pushed `feat/NNN-*` branch is the lock, not an `in-progress` ledger row, and merges go through `gh pr merge`.

- **Two seats, one bus, and both now poll.** `/next-issue` opens one PR per issue with gate frames committed under `docs/gates/pr-<issue>/`; this seat reviews and merges issue PRs via head-bound `PO-REVIEW:` sentinels. **Mail is only read at the top of a loop iteration**, so a stopped loop cannot be told to restart by mail - the bus cannot deliver the message that would make it deliver messages. Both seats now run a 10-minute in-session duty-cycle job as a stopgap (in-memory, session-local, 7-day expiry). The durable fix is #597; `scripts/agent-loop.sh` has not actually supervised the dev seat since 2026-08-01.
- **Gate frames: embed by commit SHA, not branch.** Every merged PR's frames 404 today because branches are deleted on merge (#600). This seat ruled the deviation from `CLAUDE.md`'s branch-URL wording **allowed** on 2026-08-21, narrowly: an instruction may be deviated from when it demonstrably defeats its own stated purpose, the deviation is safer on every axis, and it is named in the body. **An open issue does not otherwise license deviation** - write to the contracts and instructions as they stand on `main`.
- **The design contracts are the tiebreak, and there are now nine.** `plan/admin-design-contracts.md`: eight confirmed by the Code Owner 2026-08-20, amended 2026-08-21 (§2 splits ids by minting convention, §3 is claim-versus-capability, §6 is one model per scope). **§9 (author-authored text of unbounded length) is new and NOT covered by that confirmation** - PR #608. The POCs are never the reference; a POC is evidence of intent where a contract is silent, never an authority against one.
- **Verify claims, do not recall them.** Five instances in two days of a **right conclusion resting on evidence that was never checked**, two from each seat (#598). None was caught by review or by any gate; each was caught by someone re-reading a source while writing something else, and twice by executors refusing a brief. Two rules: quote or line-reference the file at the moment of writing, **and** check a claim about a set against the whole set, or state it as being about the part you read.
- **This checkout is stale by design, so never read it directly.** The repo-root working tree belongs to the dev loop and routinely sits many commits behind `origin/main` - it was **18 behind** on 2026-08-22. Every bare `grep`, `sed` or file read in it therefore reports a tree that is not the one anyone else is looking at, and the output is *accurate*, which is what makes it dangerous: a line number cited from here can be right about a month-old file and wrong about the shipped one (`response-browser.tsx:368` was really `:446`, caught by an executor opening the file to write a PR body). **Use `git show origin/main:<path>` or a worktree cut from `origin/main`, and `git fetch` first.** The same root cause as the diff trap below: reading a stale local tree and reporting it as current state.
- **Known trap: `git diff main..branch` on a branch that is behind** renders a sibling's merged work as deletions, which reads exactly like a revert. The honest check is the branch's own diff from its merge-base. Related: GitHub does **not** auto-retarget a PR when its base branch is deleted, so fixing what a PR contains does not fix where it points (#583, both halves).
- **Open decisions this seat cannot make**, all outside `plan/`: #600, #604, #605, #606 (instruction fixes), #607/#609 (gate hazards), and **PR #438** - the `@roonga` scope rename, 333 files, open since 2026-08-09 with no engagement, whose premise (that the `qcms` npm org is unavailable) contradicts `CLAUDE.md` and cannot be verified from this seat.

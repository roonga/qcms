...
2. **`pnpm build && pnpm typecheck && pnpm test && pnpm lint` green _inside the container_** - especially the `@roonga/qcms-db` + api Testcontainers suites. If testcontainers cannot reach its sibling containers, set `TESTCONTAINERS_HOST_OVERRIDE=host.docker.internal` (documented fallback) and re-verify.

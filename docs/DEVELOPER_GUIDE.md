...
It was diagnosed before it was fixed, because three faults look identical from a shell and need different fixes: a frozen or stepped wall clock, timers that do not wait, and a `sleep` that returns early while the clocks are fine. What the measurements found:
...
**`pnpm check:clock` tells the three apart in one command.** It times one two-second in-process wait and one `sleep 2` against both `Date.now()` and `process.hrtime.bigint()`, and names which fault it found rather than only failing. The monotonic clock is the control: a wait that is long on it and short on the wall clock isolates a stepped clock from a wait that never happened. It is deliberately **not** in `check:all` - it spends four seconds waiting and asserts a property of the machine rather than of the tree.

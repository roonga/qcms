...
**77 and 78 split that further, and the split is what tells you where to look** (issue #902). 77 means something signalled the runner: a per-command cap, an operator, or another lane. 78 means Playwright itself was killed underneath a runner nothing signalled, so nothing in the run's own tree sent it and the host is the suspect - the OOM killer, a cgroup limit, or a group signal aimed at the suite's session. Both used to arrive as one code over an `EXIT=143` that read like a red suite, which is how five kills in one run (`#902`, the #127 lane) came to have no attribution beyond a timestamp. A signalled run now also records, at the instant the signal arrived, which process was hit, its parent's command line, the seat, how far into the run it happened, and the load average and free memory; `wait` reprints the whole rc file after a 77 or a 78 so none of that is something a reader has to know to go and open. **What it cannot record is the sender.** Linux carries `si_pid` only to an `SA_SIGINFO` handler or a `signalfd` reader and Node exposes neither, so a Node signal handler genuinely cannot name its killer. A supervisor started under `setsid` normally reports `runner_ppid=1`, which is itself the useful reading: nothing in this tree is left that could have sent it.
...
So a holder is now signalled only once it is shown to be this run's, on one of two grounds:

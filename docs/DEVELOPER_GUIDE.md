...
**Telling a load timeout from a regression** (issue #932). That re-run answers "was it the budget?". Three further questions answer whose budget it was and whether it should move, and the second of them is the one this repository kept getting wrong.
...
2. **Which wall time is the margin?** This is the question that has been answered wrongly the most. A reporter duration (`--reporter=verbose`, or the JSON reporter's `duration`) counts the test's `beforeEach` and `afterEach`, and those hooks carry budgets of their own. Measured with a probe: a 1.0 s body behind a 6.0 s hook reports **7010 ms** and passes the 5000 ms per-test budget without coming near it. So a reported duration held up against the default proves nothing for any file with a container-booting hook, which in `packages/db` is most of them. Read the **body** instead, by bisecting the budget until the file stops passing:

   ```sh
   pnpm exec vitest run --project @roonga/qcms-db packages/db/src/migrations.test.ts --testTimeout=1500
   ```

   The lowest value it still passes at is what the body costs, and that is the number `CONTRIBUTING.md`'s margin check wants a small multiple of.

3. **Does it reproduce at the merge base?** Run the same file from a worktree at `origin/main`. A timeout that reproduces on a tree without your change is not your change.
4. **Does it pass alone on a quiet host?** `uptime` before and after, and re-run the one file. Evidence to record either way: the host line the harness already prints on a failed container boot (issue #812, the load and Docker census at the moment it gave up), and `pnpm ci:durations "verify (node-24)"` for whether CI's own timing for that job has moved.
...
**Where the budgets actually stood on 2026-09-18** (base `f004d19b`, 24-core host, measured alone and idle), because "several tests sit near the 5 s default" was believed for three waves without anyone re-measuring: no body in the `tooling`, `@roonga/qcms-ui` or `@roonga/qcms-db` projects was within 2x of the default. The slowest `tooling` cases (5.13 s and 4.03 s reported) already carry derived explicit budgets of 19.5 s and 30 s; the slowest `@roonga/qcms-ui` case was 1.94 s of 5 s, and `conformance.test.tsx`, named in #932, peaks at 0.36 s per case against a 3.95 s **file**; the `@roonga/qcms-db` bodies all cleared 1.5 s. The numbers that looked alarming were file totals and hook-inclusive durations. Re-measure before you widen anything.

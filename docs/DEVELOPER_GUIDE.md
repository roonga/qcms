...
Both Next apps build through `scripts/next-build.mjs`, which waits that contention out rather than reporting it as a broken build: it re-runs while the failure carries this signature and nothing else, bounded at ten minutes, and prints the pid, working directory and command line of the process holding the lock, so a wait in a log is attributed rather than mysterious. Any other failure is returned untouched on the first attempt. `QCMS_NEXT_BUILD_WAIT_MS` and `QCMS_NEXT_BUILD_POLL_MS` move the bound and the interval.

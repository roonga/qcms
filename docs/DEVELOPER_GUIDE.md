...
- **Do not trust `kill`'s exit code, and be ready to run it outside the sandbox.** A sandboxed `kill` has been seen to exit 0 while every orphan survived (issue #846, the #808 lane); it has also been seen to work. Both report success, so the exit code is not evidence either way. Check with `ss -ltnp | grep -E ':17[0-9]{3}'` afterwards, and if the listener is still there, run the kill outside the sandbox.

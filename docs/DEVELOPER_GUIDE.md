...
- **The batch export timings are passed explicitly, because OpenTelemetry JS 2.x reads none of them on its own** (issue #901). `OTEL_BSP_SCHEDULE_DELAY`, `OTEL_BSP_EXPORT_TIMEOUT`, `OTEL_BLRP_SCHEDULE_DELAY` and `OTEL_BLRP_EXPORT_TIMEOUT` are the standard names, and in 2.x the env fallbacks live only in the compatibility shims that `@opentelemetry/sdk-trace-base` re-exports, which is not the constructor any app here calls; `@opentelemetry/sdk-logs` publishes no such shim at all. Setting one of those variables against the SDK therefore changed nothing, silently. `@roonga/qcms-observability/otel` resolves all four, the three services hand the result to their processors, and a variable that is unset still resolves to the SDK's own default. If you are waiting on a dashboard and telemetry seems slow, those four are the knobs, and they work now.

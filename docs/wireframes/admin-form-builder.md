...
- **create-form `card`** (`CreateForm`, `apps/admin/app/(shell)/forms/create-form.tsx`): a `form` inside a `card`, its `fieldset` legended `New form`, holding three `text-field`s in one row - slug (required, hinted "lower-case words separated by hyphens"), title ("what a respondent sees at the top of the questionnaire"), default locale (required, defaulting to `en`) - a rejection `alert` above them, and a primary submit `button` that reads `Creating the form...` while the post is in flight (`:52-96`).

...
- **page heading**: **one `h1`, naming the version** - `Version {n}` (`forms.history.versionHeading`), passed by the route as a heading override (`.../versions/[version]/page.tsx:79-82`). Not the form's slug: two versions of one form are two screens, and a heading naming the form would give them the same one. The `h1` carries the id `VERSION_HEADING_ID` and `tabIndex={-1}`, so the body can be labelled by it and it can be a focus destination (`form-page-header.tsx:56-62`).
...
- **section nav**: the form-subtree rail, rendered in the shell's `@rail` slot beside `main` rather than inside this header, with Versions marked current (`apps/admin/app/(shell)/@rail/forms/[formId]/versions/[version]/page.tsx`). It replaced the in-header `FormTabs` when issue #561 rolled the rail across all eight form-scoped screens; the entry above named `FormTabs` until issue #614 corrected it.
- **back link**: `Back to version history` -> `/forms/{id}/versions` (`.../versions/[version]/page.tsx:84-86`).
...
- **version read failed** (a non-404 error from the version read) - the screen keeps its chrome and replaces only its body: `breadcrumb`, the `h1` naming the version, the identity line and the back link all render, with the error `alert` where the version body would be (`.../versions/[version]/page.tsx:90-98`). The rail renders as usual beside it, because the rail never reads the version. Every input the header needs survives this failure - the slug and status came from the form read, which succeeded, and the version number comes from the route params - so discarding them would cost the screen its navigation without buying any honesty. Issue #614; the shape is the one the response detail screen already ships for its own failed read (`docs/wireframes/admin-responses-ops.md`).
- **form read failed** (a non-404 error from the *form* read) - the route returns the error `alert` **alone**: no breadcrumb, no `h1`, no back link, and no rail. **Deliberately different from the state above**, and not an inconsistency to reconcile: the header is built from the form's slug and open/closed status, both of which arrived on the read that just failed, so there is no header left to render and a header captioned with a slug the app does not have would be the false claim the contract exists to prevent (`.../versions/[version]/page.tsx:61-64`; pinned by `apps/admin/app/(shell)/version-detail-read-states.test.tsx`).
- **version not found, or a version segment that is not a positive integer** - 404 (`:52,91`).

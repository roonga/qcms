...
Because the documents are derived, a policy stated once in a Zod schema is stated in both places. **Request bodies reject unknown keys** (Code Owner, 2026-09-19, issue #893): every JSON body is declared through `jsonBody` in `apps/api/src/openapi.ts`, which refuses at module load any body that is not a `z.strictObject` unless it opts out with a stated reason, so an undeclared key is a 400 `INVALID_REQUEST` naming it _and_ the generated schema carries `additionalProperties: false`. Maps whose keys are caller data stay open and keep describing their values; `POST /sessions/{id}/submit` is the one deliberate exception and says why at its route. A contract test walks every request body in both documents and in an all-flags-on composition, so a new body that is neither closed nor reasoned fails rather than shipping a schema the server disagrees with.

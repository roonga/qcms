...
- `POST /sessions/:id/submit` (session-token authed):
  ...
  3. One transaction: `insertSubmission` (lockedAnswers = `LockedSubmission.answers`, contentHash) · `markSubmitted` · `enqueue(tx, response.submitted)` with payload `{ sessionId, formId, formVersion, submittedAt, contentHash, answers }`.

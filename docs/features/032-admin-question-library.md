...
- **Editor** (per type): type picker (locked after creation - changing type is a new question by R6; explain this in the UI); label/help/required; per-type constraint editors; note: task 048 (ADR-32) later adds per-constraint validation-message fields to this editor (default-as-placeholder, blank inherits) - leave the constraint editor composable for that (bounds, pattern with live regex feedback, option list editor with stable `optionId`s auto-generated once and immutable thereafter, reorderable labels); live Zod validation surfacing 003's error paths inline.

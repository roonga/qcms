...
- **Condition editor (structured):** per-rule editing of `{ when, show }`:
  ...
  - **Live validation:** debounced calls to `POST .../draft/validate` (022) rendering the full `PublishError[]` inline - including the ADR-31 warning when a multiChoice-driven rule targets a same-step question (steer authors to next-step gating) - errors anchored to the rule/step/question they name via the structured `path`. Client-side `analyzeRuleGraph` (005 exports it) gives instant backward-target/cycle feedback before the round-trip.

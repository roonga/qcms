...
- **Reordering:** pointer drag via the handle, and keyboard (handle is a real button, accessible name per the card; Arrow Up/Down moves the row) - both driving the existing `moveOption` semantics.
...
3. Keyboard: the full rhythm above proven in a Playwright walk (including reorder via keyboard and that hover-revealed controls appear on focus without a pointer); axe green in all three modes with a cell in its editing state.
4. Reordering by drag and by keyboard both persist through save and survive a reload (e2e).

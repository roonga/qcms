...
**Stage:** 7 · **Package/App:** `@roonga/qcms-ui` (028) + `apps/portal` (029) · **Depends on:** 028, 029
...
3. R2 preserved: the new whole-step BFF route imports nothing from `@roonga/qcms-core` except types (import-surface test); it performs no rule evaluation or validation authority.
4. `@roonga/qcms-ui` renderer's native-submit mode covered by the package's conformance suite; ADR-18 respected (stored compiled doc unchanged - submittability is render-time).

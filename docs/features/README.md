...
| 047 | Portal theming: managed themes + respondent controls (mode/font/density) + declarative font registry + 4-group token contract (color/type/spacing/radius), ADR-30 - launch tier (admin theme editor split out to 049 per the 2026-07-25 amendment) | 7 | decomposed 2026-07-27 into 051 -> 052 -> 053 (row closes when 053 lands) |
| 051 | Theming A: 4-group token contract + predefined themes (L/D) + HC mode layer + per-deployment selection via config (047 EC 1-2) | 7 | in-progress (feat/051-theming-token-contract) |
| 052 | Theming B: declarative self-hosted font registry + curation config + tabular figures (047 EC 4 + font floors) | 7 | todo (after 051) |
| 053 | Theming C: respondent mode/font/density controls + persistence + SSR no-flash + brand mark from config (047 EC 3, 5-7; folds #25; UI screenshot gate) | 7 | todo (after 051 + 052; closes 047) |
...
| 054 | Observability: OTel tracing baseline (portal -> API -> pg via traceparent) + pino trace-correlated logs + SEC-13 redaction allowlist (ADR-34) | 8b | todo (Code Owner call 2026-07-29: runs immediately, beside the admin train; disjoint from 031-035) |

...
Exceptions retire when their task lands: 042, 043, 044, 045, 055, 056, 059 and 060 all carried one and are now `done`.
...
| 034 | Admin publish, preview, versions, secure links | 8a | done (PR #274; screenshot gate signed by the Code Owner 2026-08-02; preview seam per the 2026-08-02 amendment (#271); "core evaluator client-side" replaced by the 033 precedent - the API returns compiled documents plus the forward pass's visible set and `documentForVisible` moved into `@qcms/ui` so preview and portal share one projection; admin timestamps render UTC with the zone named, accepted by the Code Owner with operator-local display queued as an enhancement) |
| 035 | Admin responses, erasure, webhook operations | 8a | done (PR #284; delivery telemetry storage (migration `0012_delivery_attempt_record`, masked-before-storage HMAC) added as the minimal completion of the named delivery-detail deliverable, since 025 stored none of it; response detail cannot surface a secure link's lifecycle state (`reporting.responses` carries no `link_id`), filed as #283 rather than invented) |
...
| 048 | Author-supplied validation messages (ADR-32; folds #22) + boolean yes/no label overrides (ADR-36): per-question LocalizedText with edit-level default fallback | 8a | done (PR #313; screenshot gate signed by the Code Owner 2026-08-06) |
...
| 057 | Option grid: inline-editable table for choice options (Code Owner direction 2026-08-01; card `ds-option-grid.html` frozen before dispatch; UI screenshot gate) | 8a | done (PR #399; option-id minting deferred to the moment a row is first named, per the Code Owner ruling of 2026-08-06, and made structural rather than disciplined - `PendingRow` carries no `optionId` field and `openPendingRow` is the single door for both append and insert; remove and insert-below live in the grip's menu, per the card, which is why the menu escaping the grid's clip is a reachability requirement and not a cosmetic one; the 140px ID column against label-derived ids is left as an open card question at the screenshot gate) |
...
| 058 | Preview theme island: respondent theme/mode switch scoped to the admin preview (Code Owner direction 2026-08-01; mounts on the preview seam 034 builds, so it follows 034 rather than gating it; UI screenshot gate) | 8a | done (PR #501; **exit criterion 7's screenshot gate never fired - the Code Owner waived the screenshot gates on 2026-08-15**, and the evidence stays committed and reviewable under `docs/gates/058/`: ten frames at 390 and 1280 plus a `README.md`, including the deliberate open-overlay shot the 2026-08-14 amendment requires. Criteria 1-6 and 8 were measured, each token assertion provable-red by three recorded probes. The portalled-overlay limitation is accepted and documented rather than fixed: both routes to fixing it are fenced (a new dependency, or a `@qcms/ui` change), and the fence held) |
| 060 | Theme scope carrier: token sheets match `:is(:root, [data-qcms-theme-scope])`, treatment sheet re-anchored on the bare attribute, portal stamps it (ADR-38, Code Owner ruling 2026-08-07; unblocks 058) | 8a | done (PR #418; specificity is unchanged by construction - `:root` and `[data-qcms-theme-scope]` are both (0,1,0) and `:is()` takes its most specific argument, asserted per block rather than argued; all 14 unanchored treatment rules were anchored, not the 11 the deliverable named - the count was a transposition from `plan/058-theme-scoping-options.md:33` and three blocks would have kept matching globally; `theme-components.css`'s Tailwind `@theme` block cannot be scoped by any selector, so admin re-pins `--text-sm`/`--text-xs` to Tailwind's defaults - verified byte-identical compiled output, without which 139 admin call sites shift type scale invisibly; two ADR-38 justifications were disproved in the process and are recorded in #404, and two constraints 058 inherits are in #405) |
| 054 | Observability: OTel tracing baseline (portal -> API -> pg via traceparent) + trace-correlated JSON logs + SEC-13 redaction allowlist (ADR-34) | 8b | done (PR #181; traced e2e via in-test OTLP receiver; SEC-13 redaction; whole Playwright suite runs traced) |
...
| 059 | Erasure reaches the outbox: `outbox.payload` redacted in place and undelivered deliveries terminally cancelled (ADR-17 amendment 2026-08-02; found by the PR #284 review; after 035, before 040) | 8b | done (PR #338; the refusal is structural rather than handler-level - `claimDueDeliveries` filters cancelled and redacted rows and `resetDeliveryForRedelivery` clears neither marker, so a redacted payload has no path to the transport whatever a future caller does; 035's `deliveryTargetsErasedSession` removed in favour of `redeliveryRefusalFor` reading exactly the two columns the claim filters, taken as a breaking `@qcms/db` change while the package is unpublished) |
| 061 | Force a password change on first sign-in after bootstrap: the credential `qcms:create-admin` sets is provisional, and a server-side gate holds the account to changing it (raised 2026-08-09; not launch-gating) | 8b | todo |
| 062 | Cross-service OTLP logs: safe server logs from Admin, Portal and API correlated to distributed traces in Loki/Tempo; product-observability cleanup from #183/#184/#185/#370 | 8b | done (this PR; #184/#185/#370 resolved; #183 remains upstream-blocked) |
| 036 | Ops docs, ingress recipes, restore drill, image supply chain (the images, the solo compose topology and its full-stack CI smoke run landed early in PR #286; the task file carries only the remainder) | 8b | done (PR #344; env reference generated from and asserted against the config schema; backup/restore, ingress and enterprise recipes with a Caddy overlay; weekly restore drill that destroys the database, restores, fingerprints every table and then starts a real respondent session on a pre-dump form; `pnpm qcms:build-images` stamps a version and attaches an SBOM + provenance, asserted by walking the OCI artifact; the new Images job caught a pre-existing broken `docker compose build` of the admin image on its first run) |
| 037 | create-qcms-app CLI | 8b | todo |
| 040 | Security review and hardening (runs after 036, before 038) | 8b | in review (PR #495) - review doc (`docs/security-review-2026-08-14.md`), the SEC-3.2 matrix suite and the hardening landed; **exit criterion 3 is met**: both high-severity findings are closed, #470 by PR #480 (`fbc674f`) and #390 by PR #505 (`f30dbc5`, option B as ruled 2026-08-15), so the open high-severity count is zero and the review doc ships here. **Exit criterion 4 remains open on #360** (the npm org does not exist, so the provenance publish cannot be verified); Code Owner sign-off outstanding. **Not `todo`**: the tree already carries this work, and the branch claim dies when the branch is deleted at merge |

...
| 034 | Admin publish, preview, versions, secure links | 8a | done (PR #274; screenshot gate signed by the Code Owner 2026-08-02; preview seam per the 2026-08-02 amendment (#271); "core evaluator client-side" replaced by the 033 precedent - the API returns compiled documents plus the forward pass's visible set and `documentForVisible` moved into `@roonga/qcms-ui` so preview and portal share one projection; admin timestamps render UTC with the zone named, accepted by the Code Owner with operator-local display queued as an enhancement) |

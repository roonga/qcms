...
**Two things are pending, and neither is a model.** On **069**, the authorisation **mechanism**: whether membership and its checks live in QCMS's own `control` tables with API middleware, in better-auth's organization plugin with custom access-control statements, or in a QCMS-authored better-auth plugin. That is under evaluation against the pinned version and is ruled before the task is written; the model it carries, two role families with nothing implied, the environment scope, the per-form data scope, the administer-only installation claim, the two-person switch and the audit, is decided and does not depend on the answer. On **064**, one recommendation to confirm when the work order is written: a control-only `qcms_app_control` role beside the per-environment ones, so that "an author never reads production personal data" holds at the database as well as in the API.

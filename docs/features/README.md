...
| 036 | Ops docs, ingress recipes, restore drill, image supply chain (the images, the solo compose topology and its full-stack CI smoke run landed early in PR #286; the task file carries only the remainder) | 8b | done (PR #TBD; env reference generated from and asserted against the config schema; backup/restore, ingress and enterprise recipes with a Caddy overlay; weekly restore drill that destroys the database, restores, fingerprints every table and then starts a real respondent session on a pre-dump form; `pnpm qcms:build-images` stamps a version and attaches an SBOM + provenance, asserted by walking the OCI artifact; the new Images job caught a pre-existing broken `docker compose build` of the admin image on its first run) |

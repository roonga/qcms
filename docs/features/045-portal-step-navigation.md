...
- **Fixture-content guard:** a check that **fails the build** if the example / kitchen-sink form fixture contains health/sensitive terms - denylist (case-insensitive): `medical`, `diabetes`, `asthma`, `smok`, `health`, `pregnan`, `disease`, `diagnos`, `illness`, `prescription`. **Scope it to the example-form fixture files only** (the kitchen-sink definition + the `dev:portal` seed) - not the whole repo, since docs/ADRs legitimately discuss insurance/health in prose. Implement as a focused test in the portal e2e support suite, or a `scripts/check-fixture-domain.mjs` wired into `pnpm lint` + CI. Keeps the medical content from silently creeping back.

...
5. Switcher labels localized; keyboard operable; visible focus (standard checks).
6. Screenshot set under `docs/gates/058/`: the island in at least three theme/mode combinations against unchanged admin chrome, at 390 and 1280 (human gate).
7. `pnpm verify` + `pnpm verify:browser` green; no new dependencies; no `@qcms/ui` source changes (consumption only - if scoping turns out to require a change in how the theme sheet is structured, stop and surface it; that is a 051-contract decision, not an admin rider).

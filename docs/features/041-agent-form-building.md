...
- `POST /admin/forms/:id/draft/assist` - body: conversation turns + client state token; streams (SSE) proposal progress; the completed proposal is `{ proposedDraft, newQuestions[], rationale, issues, warnings }` where both lists come from running 022's advisory validation server-side before returning - the agent never hands the UI an unvalidated proposal silently. (The sketch originally said `issues: PublishError[]`, one list. Two things widened it and both are recorded here rather than left as a silent divergence: the API's advisory type is `PublishIssue`, the widened `PublishError | DEPRECATED_PIN` the builder already shows, and issue #123 split non-blocking `warnings` out as a channel of their own. The proposal carries what the builder carries, because it is the same kernel call over the same draft.)
...
- Slice tests with the fake provider: proposal → advisory issues and warnings attached; allowlist enforcement (a scripted rogue tool call to publish → rejected, logged); flag `none` → routes absent (404, mount-flag style); remote provider without a key → boot fails fast; local endpoint without a key → boots.

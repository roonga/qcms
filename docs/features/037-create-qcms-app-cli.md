...
The shadcn-ethos delivery vehicle: stamp the owned shell (apps, compose, config) into the adopter's repo, referencing the versioned packages (`@roonga/qcms-core`, `@roonga/qcms-a2ui-compiler`, `@roonga/qcms-db`, `@roonga/qcms-ui`) as dependencies. Per ADR-19, launch does not block on this task - if it slips, 038 proceeds with the documented manual setup from 036.

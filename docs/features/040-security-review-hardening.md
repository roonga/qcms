...
- **Token/crypto verification:** purpose-claim cross-use rejected (session token as link and vice versa); rotation overlap works for session keys, link keys and the internal token; **the webhook secret has no overlap and re-issuing it is a hard cutover** (amended 2026-08-13, issue #453: dual-signing was specified in SEC-6 and never built, so verify the documented hard-cutover and recovery sequence rather than an overlap that does not exist); tampered/expired/replayed webhook signatures rejected by the documented consumer recipe; placeholder secrets refuse boot.

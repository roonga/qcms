...
- **API (`apps/api`):** `NodeSDK` bootstrap (`@opentelemetry/sdk-node`) at the `serve.ts` entry, gated so no SDK starts when `OTEL_EXPORTER_OTLP_ENDPOINT` is unset (the OTLP exporter otherwise defaults to `localhost:4318` - the gate is ours). Server spans via `@hono/otel` middleware (it extracts inbound `traceparent` into a `SpanKind.SERVER` span; header-capture options stay unset). Instrumentations, explicit list only: `instrumentation-http`, `-undici` (outbound webhooks), and `-pg` (parameter capture off). The injected `Logger` interface is unchanged; the shared JSON logger reads the active OTel context and adds `trace_id`/`span_id` to every line when tracing is active. `requestId` is recorded as a span attribute; `x-request-id` behavior is byte-identical.
...
3. `x-request-id` semantics unchanged (inbound honored, echoed, in error envelope); API log lines carry `trace_id`/`span_id` when tracing is active via the shared logger, not call-site changes; the BFF forwards `x-request-id`.
...
5. All existing suites green with the structured logger (logger tests updated only where they asserted implementation details, never weakening the interface contract).
6. `pnpm verify` and `pnpm verify:browser` green; OTel dependencies are approved per policy and caret-pinned; `auto-instrumentations-node` is not added.
...
**Superseded scope (task 062):** Admin instrumentation and safe OTLP log export later landed as the next observability increment. This task remains the historical baseline rather than the current operator contract.

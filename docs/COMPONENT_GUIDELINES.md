...
Use this checklist when the `@roonga/qcms-ui` registry gains a control or an existing question type changes how it renders.
...
## Eight of these steps are a gate, and it names every one you missed at once

`scripts/component-registration.test.ts` (issue #91) derives the vendored component set from
`packages/ui/a2ra-manifest.json` and reports, in one failure, every place that has not heard of
every component: the fidelity manifest, the kit barrel or the renderer registry, the pinned export
set and the `PRIMITIVES` render table in `packages/ui/src/kit.test.tsx`, the admin's `"use client"`
boundary in `apps/admin/components/kit.tsx`, the Kit reality listing in
`docs/features/033-component-contract.md`, the ADR-31 commit-moment row in
`apps/portal/lib/visible.ts`, and a document to render the control in
(`packages/a2ui-compiler/golden` or `packages/ui/src/test-support`). It holds no per-component
table of its own, so nothing below becomes one more row to write, and it also checks ADR-31's own
table against the classification. What it cannot see is everything that is behaviour rather than
registration: the clear path, the conformance assertions, the keyboard walkthrough, the no-JS path,
the focus targets, the token contract and the controlled-adapter contract are still read from the
list below and proved by their own tests. A green from that gate means you are registered
everywhere, not that you are done.
...
7. **No-JS path, and the vendored control may not be able to carry it** (issues #920, #18, #974).
   Wire the native-submit serialization; there are no exceptions left to copy, because the two that existed were closed.
   **First ask where the vendored control keeps its form value.**
   react-aria routinely puts it on an element the respondent never touches: the DatePicker on an unfocusable `hidden` mirror, the NumberField on a JavaScript-synced `<input type="hidden">` beside a visible box with no `name` at all.
   With scripting off that means the keystrokes are discarded while the browser sees a satisfied form, which is worse than a refusal because nothing says so.
   When that is what you find, native-submit mode renders a **different, qcms-owned control**: `packages/ui/src/native-date-field.tsx` and `native-number-field.tsx` are the two, each one real input carrying the question's label, description and error slot, the compiled constraints as HTML attributes, and the vendored control's own style map so the two renderings of one question agree by construction.
   The scripted branch and every vendored byte stay untouched (ADR-22).
   **Then ask whether HTML can express the question's constraint at all.**
   Where it can, emit it and let the browser enforce it (the Code Owner's 2026-09-13 ruling on #920), and mind the attribute's own default while you do: an omitted `step` on a number input means `1`, so a question that admits fractions has to say `step="any"` rather than say nothing.
   Where HTML cannot - a required checkbox group is "at least one of these", which has no native encoding - drop the constraint from the no-JS render rather than approximate it, because react-aria's approximation (`required` on every box) makes the step impassable.
   Use the control's own `validationBehavior` seam so the ARIA encoding remains, and let the API's report carry the rule (Code Owner ruling, 2026-09-19, #974).
   The principle both rulings serve: **JavaScript is assumed, and the no-JS form is a fallback that must work FUNCTIONALLY, without rapid feedback.**
   Prove the markup contract in jsdom and the browser's behaviour in a `javaScriptEnabled: false` Playwright case.
   `docs/portal-constraints.md` is the authoritative statement of what the path guarantees, and it is updated in the same change - including its exception list, which is not empty (issue #988).
...
9. **Theming (ADR-30, contract in `docs/theming.md`).** Styles consume the four token groups (color/type/spacing/radius) only; the HC mode's border treatment must hold. A **vendored** control is never edited to theme it (ADR-22 byte-fidelity): its spacing, radius and type-scale wiring goes in `packages/ui/src/theme-components.css`, anchored on `[data-qcms-field]` / `[data-rac]` **beneath the scope carrier `[data-qcms-theme-scope]`** (ADR-38: every rule in that sheet is a descendant of the carrier, which is what contains it), and a new control's box needs a rule there plus a computed-style assertion in `apps/portal/e2e/theming.pw.ts`. A control that shows **digits** joins the tabular-figures selector in the same file, which reads `--type-numeric` rather than hardcoding `"tnum"`. A control must render correctly in **every** font the registry ships (`packages/ui/src/font-registry.ts`), so it never assumes a family's metrics: `apps/portal/e2e/fonts.pw.ts` sweeps all of them and re-measures the WCAG 1.4.12 floors under each. A control never writes a **font-family list** of its own: it names `var(--font-portal)` or `var(--font-mono)`, with no inline `var(..., fallback)` arguments, and the fallback tails stay in `packages/ui/src/theme.css` where one edit reaches every surface (issue #27, enforced by `scripts/check-font-tokens.mjs`).
...
13. **The adapter states the control's FORMAT, and every environment-derived attribute is accounted for (issues #151, #945).** The portal is SSR-first, so every attribute a control renders is emitted twice: once with no `navigator`, `window` or DOM, and once on the respondent's device. react-aria reads the environment for some of them, and where a question's constraint is expressible as a format it reads the format too. The NumberField is the worked example on both counts. **The format half is yours.** `@react-aria/numberfield` derives the input's `inputMode` from the resolved `Intl.NumberFormat` options as well as from platform detection, so an integer-constrained question left on the default format (three fraction digits) asked for a decimal keypad on touch and `numeric` from the server, and every reload of that step logged a React hydration attribute mismatch; stating `maximumFractionDigits: 0` for it removes the fraction digits and the divergence together. So translate the question's constraints into the control's format in the adapter rather than leaving a default that happens to be close: a constraint the control does not know about is a constraint it will contradict. **The rest of the environment half may not be yours to fix, and then you name it.** The same hook sets `aria-roledescription` unless `isIOS()`, which no adapter prop can reach, so that attribute still differs between the server and an iOS client and no Chromium project in `playwright.config.ts` can observe it. Enumerate every attribute the control's hook derives from the environment before claiming a hydration class is closed, prove the ones you can pin agree in jsdom (`packages/ui/src/number-input-mode.test.tsx` renders through `renderToStaticMarkup` with `navigator` removed, which is the branch Node takes, and overrides the platform for the client render), and give each one you cannot pin a self-arming marker plus an issue rather than silence. A browser suite with no WebKit project is not evidence about iOS.

## Decide where the behaviour is testable before you write the component

This is a design instruction, not a limitation to work around, and it binds every client component in the repository as well as the `@roonga/qcms-ui` adapters above.

...
13. **The adapter states the control's FORMAT, and it must not depend on the environment (issue #151).** The portal is SSR-first, so every attribute a control renders is emitted twice: once with no `navigator`, `window` or DOM, and once on the respondent's device. react-aria reads the environment for some of them, and where a question's constraint is expressible as a format it reads the format too. The NumberField is the worked example: `@react-aria/numberfield` derives the input's `inputMode` from the resolved `Intl.NumberFormat` options AND from platform detection, so an integer-constrained question left on the default format (three fraction digits) asked for a decimal keypad on touch and `numeric` from the server, and every reload of that step logged a React hydration attribute mismatch. Stating `maximumFractionDigits: 0` for it removes the fraction digits and the divergence together. Two rules follow. First, translate the question's constraints into the control's format in the adapter rather than leaving a default that happens to be close: a constraint the control does not know about is a constraint it will contradict. Second, when a rendered attribute is environment-derived and you cannot pin it, prove the two renders agree in jsdom (`packages/ui/src/number-input-mode.test.tsx` renders through `renderToStaticMarkup` with `navigator` removed, which is the branch Node takes) rather than waiting for a hydration warning to show up in a browser spec that happens to reload.

...
| `sleep` returns at once, so a poll loop spins and `date` seems frozen | Not the container (issue #590). Docker shares `CLOCK_REALTIME` with the host - a container time namespace can only offset the monotonic clock - and nothing in the image fakes time. Run `pnpm check:clock`: it times an in-process wait and a `sleep 2` against both clocks and names which of the three possible faults it found. Wait on the process instead of on elapsed time (`tail --pid=<pid> -f /dev/null`); [`docs/DEVELOPER_GUIDE.md`](DEVELOPER_GUIDE.md) has the measurements. |

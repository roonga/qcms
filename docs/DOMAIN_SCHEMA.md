...
**Changes from v1.2 (task 003, 2026-07-19):** §2.2 safe `pattern` subset finalized (implemented in `@roonga/qcms-core` `safe-pattern.ts`); cross-field refinements and their typed error codes listed; `QuestionVersionRecord` shape added to §4.2.
**Owner package:** `@roonga/qcms-core` (all types are Zod schemas; TypeScript types are inferred, never hand-written)

This document defines the domain model - the layer that governs *meaning*. Nothing here knows about A2UI, HTTP, or Postgres. Storage shapes live in `@roonga/qcms-db`; rendered shapes live in `@roonga/qcms-a2ui-compiler`; both derive from this model and never feed back into it.

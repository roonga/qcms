...
**What is genuinely new is the rotation path.** better-auth 1.7.3, the version this repo pins, carries a versioned key set: `secrets?: Array<{ version, value }>` (`@better-auth/core/dist/types/init-options.d.mts:518,529`). <!-- expect: secrets?: Array<{ -->
...
**better-auth 1.7.3 accepts `Origin: null` when Fetch Metadata says the request is same-origin, and the Code Owner accepted that on 2026-09-01 (issue #757, from PR #740).**
Earlier versions refused a state-changing request whose `Origin` serialized as the literal `null` outright.
1.7.2 relaxed it, and `dist/api/middlewares/origin-check.mjs` is byte-identical in 1.7.3: when `Sec-Fetch-Site: same-origin` accompanies the null origin, the library infers the origin from the request itself and validates the inferred value against `trustedOrigins` (`validateOrigin`).

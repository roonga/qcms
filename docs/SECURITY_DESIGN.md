...
- **Live sessions end.** better-auth derives its cookie-signing secret from the current version (`dist/context/create-context.mjs:76`), so promoting a new version signs every admin out. That is the correct consequence of a key change; it is not the loss of a factor.
...
- **The admin sign-in throttle** has the same property and was documented nowhere. better-auth resolves `rateLimit.storage` to `"memory"` unless a secondary storage is configured (better-auth 1.7.3, `dist/context/create-context.mjs:175`), and QCMS configures none, so each process that mounts `admin` keeps its own tally of SEC-1's three attempts per ten seconds per client address. Two such processes mean six attempts per ten seconds against an admin password.

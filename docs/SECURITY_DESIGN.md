...
The first reader recognised one spelling of a `uses:` key and walked past every other line, so nine valid YAML spellings - a flow mapping, a quoted key, a space before the colon, a block or folded scalar value, an anchored key, an explicit key, an aliased or tagged value - each carried an unpinned third-party action past the gate in silence.
The rule is now keyed on the **token**: a line that puts `uses` in key position in any spelling is either parsed as the one canonical form or refused by file and line, and the only thing still skipped is a block scalar's body, where a `uses:` is shell text rather than a reference.
So the inventory cannot shrink in silence, and a refused line is still counted.

...
**The decision is the Code Owner's; the rationale below is this repository's record of why it holds.** The ruling states the carve-out and does not argue it, so the argument is recorded here rather than attributed: the trust argument that motivates a SHA is an argument about a third party's repository, and GitHub already runs the code that would consume the pin, so a SHA there buys little against a cost paid on every bump.
**The defect this closed was live, not theoretical**: the only third-party action here is `pnpm/action-setup`, and eight `uses:` lines named it by tag - six as `@v6.0.9` and two as the moving major `@v6`, which resolved to the `v6.0.10` commit - so two jobs ran a different release of one action from the other six and nothing in the tree said which. All eight now name commit `0ebf47130e4866e96fce0953f49152a61190b271`, the `v6.0.9` release.

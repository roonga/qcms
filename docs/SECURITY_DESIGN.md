...
**The convention has three classes.** A **third-party** action (any owner but the two below) is `uses: owner/repo@<40-hex commit SHA> # vX.Y.Z`. An action published by **GitHub itself** (`actions/*`, `github/*`) stays on a version tag: that is the Code Owner's carve-out, and the reason is that the trust argument motivating a SHA is an argument about a third party's repository, while GitHub already runs the code that would consume the pin. What is still refused there is no ref at all, which resolves to the action's default branch. A **local** composite action (`./.github/actions/...`) is neither; it is reviewed in the pull request that changes it, and GitHub accepts no ref on one.
...
**What enforces it is `scripts/check-actions-pinned.test.ts`**, whose file set is derived from git and spans `.github/workflows/**` **and** `.github/actions/**`, because a composite action can call a third-party action too.
It classifies every `uses:` and fails, naming file and line, on a third-party reference that is not a 40-character lowercase hex SHA, on one pinned without a readable version comment, on a first-party reference with no ref, on one action referenced at two different pins, and on a `docker://` reference - which is a refusal with its reason rather than an omission, since Dependabot does not support `docker://` and such a reference would be a mutable pin with no updater behind it.
A `uses:` shape the reader does not understand is refused rather than skipped, so the inventory cannot shrink in silence.

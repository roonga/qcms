...
TLS terminates at the operator's ingress (ADR-20 - 036 documents a cloud-LB recipe and ships an optional auto-cert Caddy overlay); internal hops are private-network HTTP at launch (enterprise mTLS upgrade documented). HSTS at the ingress (stated in both recipes). Cookies: httpOnly + `Secure` + `SameSite=Lax` everywhere (asserted in tests, 029/031). **CSRF:** SameSite=Lax is the primary control; **both** Next apps' BFF route handlers additionally enforce Origin/Sec-Fetch-Site checks on state-changing requests, via `isSameOriginPost` in each app's `lib/server/route-helpers.ts`. The portal's copy landed in issue #487: this sentence had claimed it of both apps while only the admin had it, for four state-changing handlers, which is why the claim is now held up by a gate rather than by a convention. `scripts/check-origin-guards.test.ts` derives every state-changing route handler in every app from disk and asserts each one calls the belt by name, so an app that acquires a handler without it, or loses the belt entirely, is a red rather than a silence. Note that `Sec-Fetch-Site` is the header doing the work, not a fallback for old clients: `Referrer-Policy: no-referrer` makes a form POST serialize its `Origin` as the literal `null`, so the `Origin` comparison only ever matches on the hydrated `fetch()` path. A client sending no Fetch Metadata at all is refused. No cross-origin API exists (BFF pattern eliminates CORS entirely - no CORS headers are ever set, and their absence is a test). **Headers** (both Next apps + API): CSP (default-src 'self'; portal allows the Turnstile origin only when the adapter is active; no unsafe-inline - nonce-based if Next requires), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `frame-ancestors 'none'` (portal embedding of forms is a Phase-4 decision, not an accident). Request body size limits at the API (enforced) and at the ingress (documented in the recipes). Delivered: 017 (API headers/limits), 029/031 (apps), 036 (proxy).

...
Trivy's `trivy sbom <file>` reads SPDX too, and its own documentation is the reason it was not chosen: "Passing SBOMs generated by tool other than Trivy may result in inaccurate detection because Trivy relies on custom properties in SBOM for accurate scanning" ([Aqua Security, SBOM scanning](https://trivy.dev/latest/docs/target/sbom/)).
These SBOMs are generated by syft through `docker/buildkit-syft-scanner`, which is not Trivy, and grype is syft's sibling.
...
The syft document records the distro as a qualifier on every deb purl (`pkg:deb/debian/libc6@2.36-9+deb12u14?arch=amd64&distro=debian-12.15`), grype reconstructs it, and the report says so: measured on the three images, `debian 12.15`, with 220 of 229 matches against `deb` packages.

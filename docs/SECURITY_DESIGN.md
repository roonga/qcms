...
**The client address a per-address limit keys on is an assertion, and the chain that produces it is a trust boundary (issue #341).** Three hops, each vouching for the next: the **ingress** is the only component that sees the peer address and writes it into `X-Forwarded-For` (set, not appended, in the Caddy recipe); the **portal BFF** resolves one address from that header by counting `QCMS_PORTAL_TRUSTED_PROXY_HOPS` entries from the **right**, so entries a client wrote itself are never reached, and asserts the result on `X-QCMS-Client-Address`; the **API** reads only that header and ignores `X-Forwarded-For` and `X-Real-IP` outright, because it never faces the internet (ADR-20) and the assertion rides the SEC-4 internal-token channel, so forging it presupposes the deployment's internal token. Both failure directions are documented rather than hidden: with nothing vouched for, every caller shares one bucket (coarse, never a bucket per request); with more hops declared than exist, the resolver reads client-supplied text and per-address limiting is bypassable, which is why `docs/deploy-ingress.md` states the correct count per recipe. The address is a bucket key and nothing else - never logged, never a span attribute (SEC-13 §8a), never persisted. **The admin BFF's forwarded-address handling has not been brought onto this model** (it passes `X-Forwarded-For` through verbatim for better-auth's per-IP sign-in throttling, §2.1); 040 owns verifying SEC-1 as a system.

...
`@roonga/qcms-csv` helper and was still open at the close of this review. Adding a second
...
its _state_ moved, and only because someone else's change landed. **This does not
...
`requireAdminSessionForRequest()` _and_ the account password, so it is reachable
...
That does not change this review's reasoning about the _confidentiality_ half,
...
- **npm 2FA and provenance cannot be verified.** No `@roonga/qcms-*` package has been
  published (#360), so there is no publish to attach provenance to and no
  release account whose 2FA can be checked. See §8,
  criterion 4.
...
_authorized_ action" rather than "any request carrying it alone is refused". The
...
| 4   | `SECURITY.md` published; provenance publish verified   | **Half met.** `SECURITY.md` is published and updated by this change: private disclosure via GitHub advisories, a response commitment, a supported-versions policy and a scope statement. **The provenance half is blocked on #360: no `@roonga/qcms-*` package has been published, so `npm publish --provenance` cannot be configured, exercised or dry-run against a real registry. This is a Code Owner action and a 1.0 blocker in its own right; it is recorded as blocked, not claimed and not omitted.**                  |
| 5   | 038's pre-flight references this review doc by date    | **Met as far as 040 can meet it.** `docs/features/038-launch-gate-validation.md`'s pre-flight list now cites this document by path and date, with the note that a later pass supersedes it. Actually _running_ the pre-flight is 038's work and 038 has not run.                                                                                                                                                                                                                                                                |
...
2. **npm provenance publishing.** Nothing is published (#360).
...
| 10  | A poisoned turbo cache entry survives `--force` and fails `@roonga/qcms-db` in every worktree                                                                                                                                                                                        | **#494**                                       |

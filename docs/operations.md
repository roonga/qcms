...
| `QCMS_FLAG_AGENT_AUTHORING` | optional | `none` | Agent-assisted authoring provider (ADR-25, task 041): `none`, `anthropic`, `openai`, `google`, `openai-compatible`, or `fake` (the deterministic test provider). `none` leaves the assist routes unmounted entirely. Set it on the **admin** service too, which reads it only to decide whether the chat panel renders. Full guide: `docs/agent-authoring.md`. |
| `QCMS_AGENT_MODEL` | conditional | - | The provider's own model id. Required when `QCMS_FLAG_AGENT_AUTHORING` is not `none` or `fake`. |
| `QCMS_AGENT_API_KEY` (secret) | conditional | - | Model provider key (SEC-7 inventory row). Required when a provider is named, **except** when `QCMS_AGENT_BASE_URL` points at a local endpoint (loopback, a private-range address, `host.docker.internal`, or a `.local`/`.localhost`/`.internal` hostname) - a local runtime has no key to give. Never logged, never echoed in a boot error. |
| `QCMS_AGENT_BASE_URL` | conditional | - | Model endpoint override. **Required** for `openai-compatible` (which is how a locally hosted model is configured: Ollama, vLLM, LM Studio, llama.cpp-server); optional for the hosted providers, where it names a gateway or proxy. |
| `QCMS_AGENT_MAX_STEPS` | optional | `8` | Hard ceiling on tool-loop steps in one agent turn. |
| `QCMS_RL_AGENT_ASSIST_WINDOW_MS` / `_MAX` | optional | `60000` / `10` | Rate limit on the assist endpoint, per admin principal. |

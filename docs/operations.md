...
#### A running throttle with nothing to key on: one bucket for every administrator

There is a **second** warn line, and it fires while the throttle is on rather than off
(issue #482):

```json
{
  "level": "warn",
  "time": "2026-09-02T08:32:40.128Z",
  "service": "qcms-api",
  "enabled": true,
  "addressHeaders": "x-qcms-client-address",
  "msg": "sign-in throttling is running, but no proxy is declared in front of the admin ..."
}
```

It says the throttle is enforcing and has nothing to key on. Without a proxy the admin
BFF sees no inbound `X-Forwarded-For`, resolves no client address, and vouches for none,
so better-auth puts **every** caller in one bucket. The three attempts per ten seconds
then belong to the whole deployment: three attempts from anybody lock every administrator
out of sign-in until the window rolls, and a caller repeating them holds it closed for as
long as they care to. `/two-factor/*` behaves the same way.

That is a refusal rather than an admission, so it costs availability and not
confidentiality - but it is the property of the **default Compose stack**, which ships no
proxy, so a first deployment has it and nothing used to say so.

**The remedy is a proxy, not a variable.** Put an ingress in front of the admin hostname
(`docs/deploy-ingress.md` has two recipes), then set `QCMS_ADMIN_TRUSTED_PROXY_HOPS` on
**both** the `admin` and the `api` services, to the count that recipe calls for. The
`admin` service is the one that acts on it; the `api` service reads only whether it is
set, and that is all this line reports. It is a declaration, not an observation: the API
never faces the internet, so setting the variable there without a real proxy silences the
line and leaves the shared bucket in place. `0` counts as no proxy, because `0` means
exactly "trust no forwarded entry".

If you are running the admin somewhere it is genuinely reached without a proxy - over a
tunnel or a private network, which the `docs/deploy/fly.md` recipe does deliberately -
this line is expected and the shared bucket is the accepted cost. Note the tension it
sits in with the paragraphs above: outside production better-auth's own default would
have the limiter off entirely, and in the default production shape it is on but keyed on
a constant. Both are warned about; neither is the intended posture for a deployment
serving real administrators.
...
| `QCMS_FLAG_AGENT_AUTHORING`         | optional     | `none`                                           | Agent-assisted authoring provider (ADR-25, task 041): `none`, `anthropic`, `openai`, `google`, `openai-compatible`, or `fake` (the deterministic test provider). `none` leaves the assist routes unmounted entirely, so a request to them 404s and no chat panel renders. Set the same value on the `admin` service. Full guide: `docs/agent-authoring.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `QCMS_AGENT_MODEL`                  | conditional  | -                                                | The model provider's own model id, never interpreted here. Required when `QCMS_FLAG_AGENT_AUTHORING` names a real provider, ignored otherwise.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `QCMS_AGENT_API_KEY` (secret)       | conditional  | -                                                | Model provider key (SEC-7 inventory). Required when `QCMS_FLAG_AGENT_AUTHORING` names a real provider, **except** when `QCMS_AGENT_BASE_URL` points at a local endpoint (loopback, a private-range address, `host.docker.internal`, or a `.local`/`.localhost`/`.internal` hostname), because a locally hosted model has no key to give. Never logged and never echoed in a boot error.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `QCMS_AGENT_BASE_URL`               | conditional  | -                                                | Model endpoint override. **Required** for `QCMS_FLAG_AGENT_AUTHORING=openai-compatible`, which is how a locally hosted model is configured (Ollama, vLLM, LM Studio, llama.cpp-server); optional for the hosted providers, where it names a gateway or proxy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `QCMS_AGENT_MAX_STEPS`              | optional     | `8`                                              | Hard ceiling on tool-loop steps in one agent turn (task 041).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `QCMS_RL_AGENT_ASSIST_WINDOW_MS`    | optional     | `60000`                                          | Rate-limit window for `POST /admin/forms/{id}/draft/assist`, keyed by admin principal.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `QCMS_RL_AGENT_ASSIST_MAX`          | optional     | `10`                                             | Agent turns one administrator may request per window.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Variable                        | Required     | Default                         | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------- | ------------ | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `QCMS_FLAG_AGENT_AUTHORING`     | optional     | `none`                          | Whether the agent-authoring chat panel renders in the form builder (ADR-25, task 041). The admin reads only the *effect* of this flag, never its value in the browser, and holds no provider key: the key lives on the `api` service. Keep it in step with the API's value, or the panel appears against a surface that is not mounted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

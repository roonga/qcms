...
- **Portal.** The one client-side change is in the qcms-owned adapter: the DatePicker reads its rendered segments at the ADR-31 commit moment, because react-aria reports a value only when a date becomes complete and reports null only when every segment is empty, leaving a complete date backspaced to a partial one invisible. The vendored component is untouched (ADR-22).

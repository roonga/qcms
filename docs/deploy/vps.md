...
`.github/workflows/images.yml` **builds** all three images with SBOM and provenance (task 036) and **publishes them to GHCR** on `main` as `ghcr.io/<owner>/qcms-{api,portal,admin}`, tagged with the full commit SHA and `latest` (issue #763). ADR-20's rule is about the runtime network, not the registry: it says the API publishes **no host port** and is reachable only by the two BFFs on the internal network, which stays true however the image is distributed. The Code Owner confirmed that reading on 2026-09-02, so publishing is settled. A GHCR package is private when the first push creates it, which is what a server pull with a credential wants; leave it that way unless you decide adopters should pull anonymously. What is still missing is the deploy pipeline (issue #360). The template below is self-contained: it builds and pushes its own tag-triggered images, then pulls on the box over SSH, keeping `.env` server-side. If you would rather deploy what `images.yml` already published, delete its `images` job and pull the `ghcr.io/<owner>/qcms-<app>:<commit sha>` tag instead.

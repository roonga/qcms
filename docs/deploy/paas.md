...
| ----------- | ------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **Render**  | Fixed plans, one Blueprint file | ~ **$27**: 3x Starter instance (**$7** each, 0.5 vCPU / 512 MB) + managed Postgres Basic (~ **$6**, 256 MB) | You want a declarative `render.yaml`, predictable flat billing, and a private-service primitive. |
| **Railway** | Usage-metered, per-second       | ~ **$10-20**: $5 Hobby base + metered vCPU (~ $20/vCPU-mo) and RAM (~ $10/GB-mo) across four services       | You want to pay for actual consumption on a low-traffic deployment and like config-as-code JSON. |

Render's fixed pricing makes the bill predictable and the private-service type makes "the API has no public URL" a first-class setting. Railway is cheaper while traffic is low because it meters to the second, but the bill moves with load, so model your steady-state RAM before committing. On **both**, size the API and Postgres above the smallest tier before the smaller front ends: the API holds the only DB handle and the schedulers, and Postgres is what a squeeze starves first.

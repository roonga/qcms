...
**Membership is a set of rows, and there are two role families that imply nothing about each other.** `workspace_members` holds rows of **(administrator, workspace, role, environments, forms)**, and an administrator holds **as many rows as their job needs** rather than one row with a rank. `environments` is a set or `all`. The two families:
...
**The model above is decided; the mechanism that carries it is under evaluation, and the direction for it is recorded** (issue #995, comment 5842366437). Everything this decision states about workspaces, the two families, the environment and form scopes, the administer-only installation claim, the two-person switch and the audit is settled and does not depend on how it is implemented. What is open is **where the membership rows and the permission checks live**, evaluated read-only against `better-auth` as pinned, across three options: the `organization` plugin plus QCMS extensions, a QCMS-authored better-auth plugin, or QCMS tables with API-layer middleware.

The Code Owner's direction on that question is to keep authentication and authorisation **simple, generic and not tied to QCMS**, so that they are reusable across applications: build on the built-in `organization` plugin and add only what it lacks. What it lacks is **scope on a role**: the plugin says what a role may do to a type of thing and cannot say "handler, but only in environment `test`, only on these forms". The rest of the model is either native to the plugin or QCMS domain code under any of the three options. **The ruling follows a survey** of whether an existing better-auth plugin, or a better-auth integration with an authorisation engine, already closes that scope gap; it lands before task 069 is written, and it changes no sentence above.

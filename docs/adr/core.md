...
**Status:** decided; not built (task 064). Code Owner rulings of 2026-09-25 and 2026-09-26, issue #995: the environment set, the `webhooks` placement, the schema layout and the address shape are decided; the API's **process** shape is still awaiting confirmation (see the Notes).
...
**The data plane is isolated by a Postgres schema per environment, and the control plane has a named schema of its own.** Every QCMS object lives in a schema QCMS named; `public` is left empty. The split is exact:
...
| `env_<name>`, one per environment      | `sessions`, `answers`, `submissions`, `erasure_tombstones`, `outbox`, `webhook_deliveries`, `webhooks`                                                                                                                                               |
...
| ------------------------------------------ | --------------------- | ----------------------- |
| `sessions_form_version_fk`                 | `env_<name>.sessions` | `control.form_versions` |
| `sessions_link_id_secure_links_link_id_fk` | `env_<name>.sessions` | `control.secure_links`  |
| `webhooks_form_id_forms_form_id_fk`        | `env_<name>.webhooks` | `control.forms`         |
...
Because the environment set is configurable, the prefix is the environment's own name, which makes that name a URL path segment. It therefore has to be a legal path segment and a legal Postgres identifier inside `env_<name>`, and the environment command must refuse a name that collides with a route either surface already serves at its root. The reserved set and its derivation are task 064's, in `plan/environments-and-workspaces.md`.
...
**Note - SEC-10's role split reaches this, and the recipe stops naming `public`.** `qcms_app` holds no DDL of any kind, so it cannot create an environment schema; that is the operator command's job under the migration role. `ALTER DEFAULT PRIVILEGES` is keyed on (role, schema, object type) with no per-table filter, so `control` and each `env_<name>` need their own `USAGE` and DML grant for `qcms_app` and their own ownership by `qcms_migrate`. That is the symmetry the named control schema buys: every grant in the recipe reads "per named schema", with no schema that gets its rights by being the default one.

Three documents name `public` today and move with this decision, all in task 064: the "Least-privilege database roles" recipe in `docs/operations.md`, whose row says `qcms_migrate` "owns `public` and every object in it" and whose grants are written `IN SCHEMA public`; the SEC-10 paragraph in `docs/SECURITY_DESIGN.md` section 7, which says `qcms_app` gets "no `CREATE` on `public`"; and the sentence in `docs/reporting-view.md` that `qcms_reporting` "deliberately gets **no** privileges on the `public` schema", which becomes `control` and the environment schemas, since those are where the raw ledger, tokens and auth tables then live. All three are correct as they stand and are corrected in the change that moves the tables, not before. `apps/api/e2e/security/03-db-least-privilege.e2e.ts` is where the result is asserted rather than read.
...
Whether there is **one** `qcms_app` across every environment or one per environment is Q17 and is open. Under the named-schema layout that question reads more cleanly than it did: the app role's rights are `control` plus its own environment schema and nothing else, so "one role per environment" becomes a difference an operator can see in a grant listing rather than an argument about resolution defaults.
...
**Note - the address shape is decided; the API's process shape is awaiting Code Owner confirmation.** The `/<env>/` prefix above settles the address half: it is the shape on both surfaces, it is a mountable group in ADR-09's sense so an operator can leave a non-prod environment unmounted and get a 404 rather than a refusal (that record's own Note records the second mount this needs, since anonymous entry is not separately mountable today), and prod stays unprefixed. What is **not** settled is the process half: how one deployment serves more than one environment. The leaning on record is **one API process holding one connection pool per environment**, with the request's environment, read from the prefix, selecting the pool. ADR-20 is unchanged by that shape, which is part of its appeal: it does not add a standing container. Recorded as the leaning and marked awaiting confirmation in `plan/environments-and-workspaces.md` (Q2); task 064 may follow it provisionally, and everything else this decision states holds under either shape.
...
**Note.** The rest of what this decision does not settle is section 7 of `plan/environments-and-workspaces.md`, open questions for the Code Owner, cited by number from here and from the task rows in `docs/features/README.md`; section 6 of the same document records the questions that are answered, with their answers.

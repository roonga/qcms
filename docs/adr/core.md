...
**Status:** decided; not built (task 064). Code Owner rulings of 2026-09-25 and 2026-09-26, issue #995, the last of them after the Code Owner scenario-tested this design against a three-environment, two-workspace deployment. Nothing here is open; `plan/environments-and-workspaces.md` section 6 records all thirty questions with their answers, and section 7 records the scenario test.
...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------- |
| **Authoring** | **editor** (author drafts, publish versions), **approver** (approve and release, configure endpoints and mint links in an environment), **owner** (membership and workspace settings) | the control plane           | workspace-wide                            |
...
**The model above is decided; the mechanism that carries it is under evaluation.** Everything this decision states about workspaces, the two families, the environment and form scopes, the administer-only installation claim, the two-person switch and the audit is settled and does not depend on how it is implemented. What is being evaluated against the pinned `better-auth` is **where the membership rows and the permission checks live**: QCMS's own `control` tables with API-layer middleware, better-auth's organization plugin with custom access-control statements, or a QCMS-authored better-auth plugin. That is a mechanism ruling the Code Owner takes separately, before task 069 is written, and it changes no sentence above.

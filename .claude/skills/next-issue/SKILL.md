...
1. Stop if the shared checkout is dirty. Inspect open `fix/*` PRs and claims before selecting fresh work. A current-head `AGENT-REVIEW: CHANGES-REQUESTED` cycle outranks new work; a stale verdict needs fresh review; a current approval proceeds to final merge checks.
...
4. Spawn `task-executor` in an isolated worktree with the issue body and relevant authoritative references. Prepare only the human-review material explicitly required by the issue.
5. Rebase completed work onto current `origin/main`, run applicable gates, push, and open one PR for the issue. Use a Conventional Commit title, checked acceptance criteria, `Fixes #NN`, the executor report, and `## Retro`. Append non-empty friction to `docs/RETRO.md` on the branch before final review.

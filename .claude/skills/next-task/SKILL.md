...
2. Read the ledger (`docs/features/README.md`). Candidate = lowest-numbered `todo` task whose **Depends on** rows are all `done`, honoring the ordering exceptions:
...
4. **Stale-claim recovery (interrupted sessions - usage limits, crashes).** If a task is `in-progress` and no live session owns it, resume it before starting anything new:

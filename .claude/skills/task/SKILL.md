...
4. **Prepare the PR.** When implementation is done, rebase onto current `origin/main`, run the applicable gates on that rebased tree, and push. **A push is evidence only when the remote ref actually moved**, at every push in this flow: `git symbolic-ref -q HEAD` before it, and `git ls-remote origin <branch>` against `git rev-parse HEAD` after it, because a commit made on a detached HEAD leaves the branch ref behind while `git push` answers `Everything up-to-date` (issue #567). **A post-rebase push uses `git push --force-with-lease=<branch>:<the sha last pushed>`**, never a bare `--force`, and a refused lease means the remote moved and wants reading rather than overriding (issue #933). The executor never opens the PR (`.claude/agents/task-executor.md` rule 9): it leaves a body at `node scripts/agent-scratch.mjs pr-body.md`, with each `Fixes #NN` on its own line, and the conductor opens the PR from that file. That PR carries a Conventional Commit title, checked exit criteria, any `Fixes #NN` line, the executor report, and a `## Retro` section. Add the PR number to the ledger row and append non-empty friction to `docs/RETRO.md` on the same branch, then push and wait for required CI.

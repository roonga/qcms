...
Report, in order: **VERDICT: APPROVE / REJECT** · exit-criteria table (criterion → MET/UNMET → evidence) · rule findings (file:line, which rule, why) · scope findings. Be specific enough that the orchestrator can relay fixes without re-deriving them. Approve only when every criterion is MET and there are zero rule violations.

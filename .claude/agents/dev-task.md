...
- **Gates before you call anything done:** `pnpm --filter <pkg> lint` first on new files (cheapest leg, likeliest to fail), then the gates proportional to what you touched - up to `pnpm verify` at root, plus `pnpm verify:browser` when `apps/portal`, `apps/admin`, or `@qcms/ui` changed. Force-run Docker-backed suites (`pnpm exec turbo run test --force`) and confirm `0 cached` before trusting a test phase.
- **Trust the repo over memory:** read `docs/PROJECT_INSTRUCTIONS.md` (R1-R7), the ledger, and `git log` before asserting project state. Out-of-scope discoveries become notes in your report, never silent code.

...
7. Finish state, exactly one of:
   - **Done:** all exit criteria pass and `pnpm build && pnpm typecheck && pnpm test && pnpm lint` is green at root (typecheck is part of the gate - CI runs it; build alone does not catch test-file type errors). Format files you created before the gate (root lint includes a Prettier check). Commit on the task branch (Conventional Commits, task number in message). Report: exit-criteria checklist with evidence per item, files changed, suites run, discoveries for issues.

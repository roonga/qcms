...
`@roonga/qcms-ui/kit` now exports `Menu` and `MenuItemEntry` alongside the other vendored
controls.
...
`react-aria-components` an import of `@roonga/qcms-ui` alone, which is the property
